...
try {
    $stmt = $conn->prepare("SELECT `first_name`, `last_name` FROM `users` WHERE `first_name` = :first_name AND `last_name` = :last_name");
    $stmt->execute([
    ...
    if (empty($nameExist)) {
        ...
        $updateStmt = $conn->prepare("UPDATE `users` SET `first_name` = :first_name, `last_name` = :last_name, `contact_number` = :contact_number, `email` = :email, `username` = :username, `password` = :password WHERE `tbl_user_id` = :userID");
